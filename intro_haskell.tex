...
\note[itemize]{
\item {type let \texttt{f x = x + 1} on the command line and then \texttt{t: f}}
\item {type let \texttt{\textbackslash g = x -> x + 1} on the command line and then \texttt{t: g}}
}
...
\note[itemize]{
\item {type  \texttt{t: +}}
\item {\texttt{+} is an infix operator but we can make it prefix: type \texttt{(+) 1 2}}
\item {type \texttt{:t (+)}}
\item {type \texttt{:t (+) 1} and \texttt{:t (+1)}}
\item {type \texttt{:t (+) 1 2} and \texttt{(+1) 2}}
}
...
\note[itemize]{
\item {type  \texttt{0:[], 1:0:[], 0:[1,2,3]}}
\item {type \texttt{[1,2,3] ++ [4,5,6]}}
}
